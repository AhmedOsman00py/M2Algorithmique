...
Rcpp::List dijkstra(NumericMatrix graph, int source, int dest) { 
  ...
  return Rcpp::List::create( _["Distance"]=dist[D],_["trajet"]=path);
...
void affichage(int source, int dest, Rcpp::NumericVector dist, Rcpp::NumericMatrix graph){
    int V = dist.size();
    int D = dest;
    std::vector<int> path ; 
    path.push_back(dest); 
    ...
    Rcpp::Rcout<<"Le plus court chemin entre "<<source + 1<<" et "<< D+1 <<" est : "; 
    while (!path.empty()) { 
        Rcpp::Rcout<<path.back() + 1 <<"  "; 
        path.pop_back();
    }
...
void Dijkstra(Rcpp::NumericMatrix graph, int source) { 
    
    int V = graph.nrow();
    source = source - 1;
    
    Rcpp::NumericVector dist(V); 

      
    Rcpp::LogicalVector visited(V);  
    ...
    for(int dest = 0  ; dest <V ; dest++){
        if(dest!=source and dist[dest]!=INT_MAX){
            affichage(source,dest, dist, graph);
        }
        if(dist[dest] == INT_MAX){
            Rcpp::Rcout<<"pas de chemin possible entre "<< source + 1 <<" et " << dest + 1 ;
        }
    }
